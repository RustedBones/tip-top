...
/**
  * This is an event sourced entity. It has a state, [[TiptopState]], which
  ...
  */
class TiptopEntity extends PersistentEntity {

  override type Command = TiptopCommand[_]
  override type Event = TiptopEvent
  override type State = TiptopState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: TiptopState = TiptopState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case TiptopState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        TiptopState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class TiptopState(message: String, timestamp: String)
...
object TiptopState {
  /**
    ...
    */
  implicit val format: Format[TiptopState] = Json.format
}

/**
  * This interface defines all the events that the TiptopEntity supports.
  */
sealed trait TiptopEvent extends AggregateEvent[TiptopEvent] {
  def aggregateTag = TiptopEvent.Tag
}

object TiptopEvent {
  val Tag = AggregateEventTag[TiptopEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends TiptopEvent
...
/**
  ...
  */
sealed trait TiptopCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends TiptopCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends TiptopCommand[String]
...
/**
  ...
  * application loader.
  */
object TiptopSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[TiptopState]
  )
}
